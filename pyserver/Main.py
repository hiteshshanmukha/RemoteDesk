...
class ServerMain:
    ...
    def ban_selected(self):
        ...
        for item in selected:
            ...
            if ip not in self.settings['banned_ips']:
                ...
                messagebox.showinfo("IP Banned", f"IP {ip} has been banned")
            else:
                messagebox.showinfo("Already Banned", f"IP {ip} is already banned")
                
        # Refresh the list
        self.refresh_clients()
    
    def update_status(self):
        """Update the status display and refresh clients list periodically."""
        if self.server_running and self.server_instance:
    ...
    def on_close(self):
        ...
        if self.server_running:
            result = messagebox.askquestion("Stop Server", 
